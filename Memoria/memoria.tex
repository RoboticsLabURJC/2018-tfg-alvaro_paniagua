...
\begin{figure}[h]
  ...
  \caption{Esquema que presenta la estructura de WebSim.}
...
es el propio simulador robótico.\\
...
La arquitectura como se puede apreciar en la figura ~\ref{fig:arquitectura} sigue un diseño por módulos,
...
\begin{itemize}
	\item La dependencias más importante  de \emph{WebSim} es el 
	entorno \emph{AFRAME}. Para la simulación del robot en el entorno ha sido necesario crear tres componentes 
	llamados \emph{followBody, spectatorComponent, intersectionHandler} y es \emph{WebSim} el encargado de 
	...
	interfaz en lenguaje JavaScript llamada \emph{HAL API} \emph{(Hardware Abstraction Layer)} lo que simplifica
	mucho el uso del robot creado en AFRAME, podemos enviarle instrucciones que el robot ejecutará mediante una 
	única línea de código, lo que permitirá al usuario crear una lógica para el robot de una manera limpia sin
	...
	\item Otra funcionalidad importante de WebSim es controlar la ejecución del
	\emph{mundo}, es decir, permite arrancar o pausar el robot. Se muestra en la figura ~\ref{fig:arquitectura} 
	...
	externo puede acceder a los sensores y actuadores del robot simulado y gobernar su comportamiento. El 
	software externo por tanto se encargará únicamente de enviar el código con la lógica del robot y será
	\emph{WebSim} el encargado de ejecutar dicho código.	
...
de colores y evitar obstáculos mediante el control de posición. 
...
Ahora se describirán cada una de las partes del HTML de la figura ~\ref{fig:escena-simulador}, se 
explican las que no sean entendibles de manera directa adjuntando una imagen para poder ver de manera
más clara la parte del código al que se hace referencia.\\
...
Como hemos comentado en el capítulo ~\ref{chap:herramientas} la etiqueta \emph{a-scene} 
(Figura ~\ref{fig:a-scene}) es la etiqueta principal utilizada en \emph{AFRAME} para generar la visualización, en
nuestro caso como queremos una escena simple le hemos dado un color de fondo gris y hemos activado las físicas y 
mediciones. Las físicas permite hacer uso del motor de físicas en el paquete \emph{aframe-physics}
en el cual existe la gravedad, rozamiento y otros tipos de fuerzas. La etiqueta \emph{stats} nos 
...
\begin{figure}[h]
	...
	\caption{Etiqueta \emph{a-assets}, es la etiqueta usada por \emph{AFRAME} para la gestión de texturas y 
	recursos externos.}
	\label{fig:a-assets}
\end{figure}
...
la funcionalidad que requiramos. Haciendo uso de ésto hemos podido simular nuestro robot, nuestro robot
se compone de ruedas, sensores y cámara. La Figura ~\ref{fig:tags-pibot} muestra las etiquetas y atributos
utilizados para la configuración inicial en la simulación del robot.\\
...
Para la simulación del cuerpo del robot hemos utilizado el atributo \emph{collada-model} que nos 
...
al cargarlo en la escena. Además se ha utilizado el atriburo \emph{dynamic-body}, éste atributo lo 
que hace es dotar de físicas a ese elemento con la particularidad que el elemento puede ser movido
por otros, en la imagen también se ve el atributo \emph{static-body} que también es dotado de físicas
pero de una manera distinta, las entidades con éste atributo no pueden ser movidos, se usa en general
para el suelo, paredes y en general entidades que no han de ser movidas en la escena. En la figura 
~\ref{fig:pibot-simulado} se muestra el renderizado final del robot en la escena.
...
El robot además tiene sensores de ultrasonido, para los sensores de ultrasonido se ha 
utilizado el atributo \emph{raycaster} que se encapsula dentro de la entidad con el identificado
\emph{positionSensor} (Figura ~\ref{fig:pos-sensor}) junto con el componente \emph{followBody} que se muestra en 
la figura ~\ref{fig:raycasters}. La carga de estos elementos
...
Por último tenemos la cámara, como vemos en la figura ~\ref{fig:pibot-simulado} es un elemento hijo encapsulado 
dentro de una entidad, esto se hace así debido a las recomendaciones de \emph{AFRAME} que indican que para 
modificar la posición de la cámara incluyamos esta dentro de una entidad. La etiqueta cámara cuenta con 
atributos entre los que destacaremos dos \emph{spectator y wasd-controls-enabled}.
...
En esta subsección se explica qué tipo de etiquetas se han utilizado para la generación de los elementos
...
En el siguiente apartado se explica cada una de las partes del objeto robot \emph{(HAL API)} su
funcionalidad y cómo ha sido implementado, las partes que se implementan se encuentran 
enumeradas en la sección ~\ref{sec:objetivo}. \\
...
En el robot tenemos dos hebras independientes, la hebra de los motores y la hebra de recogida de imagen de la
cámara. Son funciones autónomas que se llaman así mismas cada cierto tiempo extrayendo o añadiendo datos
en otras variables internas del robot, lo que permite que podamos hacer uso de estos datos de manera asíncrona.
...
\begin{itemize}
	...
	\item \emph{defaultNumOfRays}: Es una variable de configuración de los sensores de ultrasonidos,
	...
	\item \emph{this.robot}: Es una variable que permite crear un link entre la abstracción de 
	...
	\item \emph{this.activeRays}: Es una variable de control de tipo \emph{boolean} que nos permite
	...
	\item \emph{this.understandedColors}: Es una variable interna del robot que permite mapear 
...
\begin{itemize}
	\item \emph{getV}: Método para que el usuario pueda conocer la velocidad lineal actual configurada
	en el robot.
	
	\item \emph{getW}: Método para que el usuario pueda conocer la velocidad angular actual
	configurada en el robot.
	
	\item \emph{getL}: Método para que el usuario pueda conocer la velocidad de elevación 
	configurada en el robot.
	
	\item \emph{setV}: Método para que el usuario configure la velocidad lineal del robot.
	
	\item \emph{setW}: Método para que el usuario configure la velocidad angular del robot.
	
	\item \emph{setL}: Método para que el usuario configure la velocidad de elevación del robot.
	
	\item \emph{move}: Método que combina la funcionalidad de \emph{setV} y \emph{setW}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm, height=8.5cm]{img/EJECUCION-MOTOR.PNG}
...
Finalmente se muestra una tabla con las funciones que se ofrecen en el HAL API para hacer uso de los motores
...
\subsection{Sensor de distancia, ultrasonidos}

El sensor de ultrasonido permite detectar obstáculos abarcando un 
...
\begin{itemize}
	...
	\item \emph{getDistance y getDistances}: Estas dos funciones son las que se sirven para el 
...
simulación de estos sensores se ha hecho uso del sistema de coordenadas y rotación de \emph{AFRAME}. De
esta manera hemos podido desarrollar dos funciones que simplifican el acceso a los datos, estas funciones
...
\begin{itemize}
	...
	\item \emph{getImage}: Esta función la usará el usuario final para obtener los datos de la cámara
...
Como se ha comentado una de las funcionalidades de \emph{WebSim} es el control de ejecución de código 
en el robot simulado. Para esto se utiliza un mecanismo simple y es una variable dentro del simulador de tipo
...
Si el robot no tiene ningún código ejecutando \emph{WebSim} permite la ejecución del código mediante el 
...
\section{Conexiones con software externo}
\label{sec:conexion}

Como bien se ha señalado en la sección ~\ref{sec:arquitectura}, \emph{WebSim} permite la conexión con 
software externo. Esta funcionalidad se implementa aprovechando una de las características principales
...
En esta sección se explica el empaquetado final de \emph{WebSim}. Para empaquetarlo se ha hecho 
...
el código de la aplicación una vez empaquetado a modo de optimizar el rendimiento para cargar la página
...
dependencia. Lo que se ha hecho es desde el archivo \emph{websim.js} importar las dependencias de
...
La figura ~\ref{fig:resolve} muestra la configuración en \emph{WebPack} que declara cómo ha de sen resueltos los
...
\begin{figure}[h]
	...
	\caption{Declaración de la configuración del servidor estático para desarrollo que \emph{WebPack} nos
	permite usar.}
	\label{fig:devserver}
\end{figure}

La figura ~\ref{fig:devserver} nos muestra la configuración de puertos y la dirección IP del host donde 
se va a levantar un servidor estático de desarrollo por la herramienta \emph{WebPack}. Esto nos permite hacer
