...
\begin{itemize}
	...
	\item Ofrece la variable que contiene el objeto robot \emph{(myRobot)} de manera que el usuario
	no tiene que instanciar ningún tipo de variable de la clase \emph{RobotI} ya que se le ofrece para
	el uso directo. Esto evita que se puedan crear varias instancias del mismo objeto de AFRAME lo que 
	llevaría a una ejecución inadecuada además de simplificar a cualquier usuario el uso del simulador.
	...
	está contenido en la variable global de \emph{WebSim} llamada \textbf{myRobot}.
...
Los ejercicios a resolver serán los siguientes:

\begin{itemize}
	\item Ejercicio sigue-línea: Este ejercicio se basa en aprender a programar una lógica haciendo 
	uso de los motores y los sensores infrarrojos en los que el usuario tomara los datos del sensor
	infrarrojo y emitirá una serie de instrucciones en función de el valor obtenido. Se trata de seguir
	una línea de un color predefinido.
	
	\item Ejercicio choca-gira: Este ejercicio se basa en que el usuario haga uso de los sensores
	de ultrasonido para programar una lógica que evite que el robot se choque con los obstáculos.
	
	\item Ejercicio sigue-objeto: Este ejercicio se basa en que el robot mediante el uso de su sensor
	más potente, la cámara, sea capaz de filtrar un objeto en la escena por su color y perseguirlo
	de manera constante.
\end{itemize}


A continuación se da un enlace al canal de Youtube de la plataforma \textbf{JdeRobot} donde se muestran
varios vídeos mostrando los ejercicios anteriormente mencionados.
...
se ha podido intuir a lo largo del documento no existe de un servidor con una gran funcionalidad, al 
contrario, el servidor utilizado es un servidor estático que únicamente sirve páginas HTML, archivos
JS y CSS.
...
En el siguiente enlace también se ven tutoriales de uso de las distintas interfaces de la interfaz 
que se ofrece al usuario.

\url{https://www.youtube.com/channel/UCgmUgpircYAv_QhLQziHJOQ?view_as=subscriber}\\

Por último en el archivo README del enlace a continuación se muestra un manual completo de uso de
la API que se ofrece para el robot (en inglés).
