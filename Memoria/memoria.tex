...
de el crecimiento de la realidad virtual en la web mediante el framework \textbf{AFRAME}.\\
...
que un humano, detectar los distintos objetos que conforman nuestro entorno, conducción
autónoma (coche Tesla) e incluso detectar a las distintas personas de una sala. Este nuevo paradigma 
acarrea la necesidad de formar a nuevos expertos en éste área lo que conlleva la formación desde edades 
más tempranas y la necesidad de tener herramientas que desarrollen el aprendizaje accesibles para
cualquier persona.\\
...
programación y ya de pequeño tuve mis primeras experiencias en robótica haciendo robots de \textbf{Lego} 
usando \textbf{Lego Mindstorm.}\\
...
\begin{itemize}
  ...
  \item En el capítulo~\ref{chap:Websim-JS} se muestran ejercicios que el alumno podrá resolver
  usando el lenguaje \emph{JavaScript}.
  
  \item En el capítulo~\ref{chap:WebSim-Blockly} se muestran ejercicios a resolver por el alumno
  usando el lenguaje de bloques visuales \emph{Blockly}.
...
debido a la aparición de frameworks como \emph{React, VueJS y AngularJS} que implementan
...
que nos concierne de ahí la elección de jQuery por encima de estos otros frameworks.
...
En la siguiente sección se explicará en profundidad el papel que desempeña \emph{WebSim}, su
funcionalidad.\\
...
\begin{itemize}
	...
	\item Por último como se ha comentado una de las dependencias de \emph{WebSim} es el framework
...
\subsection{Objeto Robot - \emph{RobotI}}
\label{subsec:robot}
...
la velocidad configurada por el usuario.\\
...
\begin{itemize}
	...
	se llama a si misma la función pasados 33 milisegundos (30 FPS).
...
La interfaz del simulador puede ser variable, como hemos explicado el objetivo es que podamos conectar
distintos tipos de editores sin tener que hacer modificaciones en el \emph{core} de \emph{WebSim}.\\

El bloque del editor de código se encargará de la configuración del editor y módulos extra necesarios 
para funcionar con WebSim, esto lo incluimos ya que en el caso de \emph{Blockly} tenemos que crear
bloques personalizados para usar bloques que se conecten con \emph{WebSim}.

El editor se encargará de recoger el evento del botón \emph{play} en la interfaz de usuario y una vez
ocurra el evento, obtener el código del editor y enviarlo mediante un evento que \emph{WebSim} se 
encuentra escuchando. El evento que escucha \emph{WebSim} es el evento \emph{'code-to-run}, por tanto, 
el editor es el encargado de enviar el evento junto con el código que quiere que se ejecute.

En la figura ~\ref{fig:blockly-setup} se muestra una imagen con la función que configura \emph{Blockly}
en nuestra aplicación.

\begin{figure}[h]
	\centering
	\includegraphics[width=6.5cm, height=8cm]{img/blockly-setup.png}
	\caption{Ejemplo de código que configura el editor de Blockly.}
	\label{fig:blockly-setup}
\end{figure}

En general la parte del editor se encargará de interactuar con los botones de la interfaz necesarios,
\emph{WebSim} únicamente es el núcleo de la simulación y no se encarga de interactuar con la interfaz
de usuario.\\

En la figura ~\ref{fig:get-code} se muestra en el caso de Blockly cómo se obtiene el código y cómo 
se envía a \emph{WebSim}.

\begin{figure}[h]
	\centering
	\includegraphics[width=10cm, height=3cm]{img/get-code.png}
	\caption{Ejemplo de código que obtiene el código del editor y lo envía a WebSim para que éste lo
	ejecute.}
	\label{fig:get-code}
\end{figure}
...
\begin{itemize}
	...
	no soportan el framework AFRAME.
\end{itemize}
...
\end{itemize}
\newpage
\section{API del Robot, \emph{HAL API}}
\label{sec:API}
...
\endhead
%
myRobot.setV(velLineal) & \begin{tabular}[c]{@{}c@{}}Mueve hacia delante o atrás el robot.\\ INPUT:\\ - velLineal: numero con la velocidad lineal.\end{tabular} \\
myRobot.setW(velAngular) & \begin{tabular}[c]{@{}c@{}}Hace girar al robot.\\ INPUT:\\ - velAngular: numero con la velocidad angular.\end{tabular} \\
myRobot.setL(velElevacion) & \begin{tabular}[c]{@{}c@{}}Hace que el robot se mueva hacia arriba.\\ INPUT:\\ - velElevacion: numero con la velocidad de elevación\end{tabular} \\
myRobot.move(velLineal, velAngular) & \begin{tabular}[c]{@{}c@{}}Mueve el robot hacia delante/atrás y gira al mismo tiempo.\\ INPUT:\\ Los mismos parámetros que en setV y setW.\end{tabular} \\
myRobot.getV() & \begin{tabular}[c]{@{}c@{}}Obtener la velocidad lineal configurada en el robot.\\ OUTPUT:\\ number\end{tabular} \\
myRobot.getW() & \begin{tabular}[c]{@{}c@{}}Obtener la velocidad angular configurada en el robot.\\ OUTPUT:\\ number\end{tabular} \\
myRobot.getL() & \begin{tabular}[c]{@{}c@{}}Obtener la velocidad de elevación configurada en el robot.\\ OUTPUT:\\ number\end{tabular} \\
myRobot.getImage() & \begin{tabular}[c]{@{}c@{}}Obtener la imagen de la cámara en el robot\\ OUTPUT:\\ cv.Mat() con la imagen de la cámara del robot.\end{tabular} \\
myRobot.getObjectColor(color) & \begin{tabular}[c]{@{}c@{}}Devuelve un objeto con datos sobre el objeto\\  que detecte la cámara con el color pasado \\ como parámetro devuelve un objeto \\ del siguiente tipo \\ INPUT:\\ -color: color como string.\\ OUTPUT:\\ \{center: {[}cx, cy{]}, area: areaInt \}\end{tabular} \\
myRobot.getObjectColorRGB(filtroBajo, filtroAlto) & \begin{tabular}[c]{@{}c@{}}Devuelve un objeto con datos sobre el objeto\\  que detecte la cámara con el color pasado\\  como parámetro devuelve un objeto del siguiente tipo \\ INPUT:\\ - filtroBajo: lista de longitud 4 con valores 0 a 255 (RGBA)\\ - filtroAlto: lista de longitud 4 con valores de 0 a 255 (RGBA)\\ OUTPUT:\\ \\ \{ center: {[}cx, cy{]}, area: areaInt \}\end{tabular} \\
myRobot.getRotation() & \begin{tabular}[c]{@{}c@{}}Devuelve un objeto con la rotación del robot en los 3 ejes\\ OUTPUT:\\ \{\\ x: rotacionX\\ y: rotacionY\\ z: rotacionZ\\ \}\end{tabular} \\
myRobot.followLine(filtroBajo, filtroAlto, velocidadLineal) & \begin{tabular}[c]{@{}c@{}}Simplificación del algoritmo del sigue lineas\\ INPUT:\\ Se utilizan los mismos inputs que para los metodos \\ getObjectColorRGB() y setV()\end{tabular} \\
myRobot.readIR(color) & \begin{tabular}[c]{@{}c@{}}Permite obtener valores entre 0-3 que simulan \\ el uso de sensores infrarrojos para un \\ color de linea pasado como parámetro\\ INPUT:\\ -color: color a filtrar en la imagen como string\end{tabular} \\
myRobot.getDistance() & \begin{tabular}[c]{@{}c@{}}Permite obtener la distancia del objeto que tiene delante\\ OUTPUT:\\ number\end{tabular} \\
myRobot.getDistances() & \begin{tabular}[c]{@{}c@{}}Permite obtener la distancia de los objetos detectados\\  en un arco de 180º, devuelve 31 valores por defecto\\ OUTPUT\\ Lista con 31 valores de tipo number.\end{tabular} \\
myRobot.getPosition() & \begin{tabular}[c]{@{}c@{}}Permite obtener la posición del robot en la escena\\ OUTPUT:\\ \{ \\ x: coordenadax\\ y: coordenaday\\ z: coordenadaz\\ theta: rotacion eje Y (horiz)\\ \}\end{tabular}
\end{longtable}
