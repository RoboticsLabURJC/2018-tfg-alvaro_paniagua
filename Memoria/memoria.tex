...
un simulador con peso computacional únicamente en el cliente (\emph{WebSim}), es decir, el navegador. 
El simulador aprovecha el crecimiento de las tecnologías web y más específicamente el crecimiento de 
\emph{AFRAME}.\\

Para el uso del simulador se han desarrollado además dos páginas web que permiten editar programas robóticos
en el propio navegador en lenguaje JavaScript y lenguaje visual de bloques con Blockly para ejecutarlos en el
propio simulador. Estas aplicaciones web son usadas por la plataforma \emph{JdeRobot Kids} para enseñar
a estudiantes las bases para resolver ejercicios de visión artificial y programación robótica.\\


Para el desarrollo del simulador \emph{WebSim} se han utilizado diversas herramientas como, 
\emph{AFRAME, AFRAME Physics (sistema de físicas de AFRAME), HTML5, JavaScript, jQuery, CSS3, Blockly y ACE
...
This project is fits on web technologies and robotics, it intends to develop
a simulator with the computational weight in the client (WebSim), the browser.
The simulator takes advantage of the growth of web technologies and more specifically the growth of
AFRAME.\\

To use the simulator I developed two web pages that allows the user to write robotic programs directly on
the browser using JavaScript language and Blockly as visual blocks language to execute the program on the
embedded simulator. This web aplications are used by JdeRobot Kids platform to teach students the basics to
solve artificial vision and robotic programming exercises.\\


Different tools have been used to develop the \emph{WebSim} simulator as \emph{AFRAME, AFRAME Physics system
...
La motivación de este proyecto es la de sustituir el presente simulador \emph{Gazebo} utilizado por
la plataforma educativa \emph{JdeRobot-Kids} ya que éste tiene un gran peso computacional en el servidor. Se 
busca la creación de un simulador con peso únicamente en el lado cliente lo que permite escalar a un mayor 
...
Dentro del marco de la robótica educativa cabe hablar de la plataforma \emph{JdeRobot-Kids} en la cual
se enmarca el presente proyecto. Esta plataforma ofrece las herramientas necesarias para la 
enseñanza de robótica en alumnos de secundaria. La plataforma tiene una intención de potenciar los 
...
Python y los robots utilizan el hardware de \emph{Raspberry PI}. La propuesta educativa llevada acabo 
por la plataforma se ha puesto en práctica con éxito en la Fundación Franciscanas de Montpillier. 
Actualmente la plataforma cuenta como infraestructura hardware con el robot \emph{PiBot} el cual cuenta
con los siguientes componentes, cámara, sensores de ultrasonido, sensores infrarrojos y dos 
servomotores todo ello conectado a la \emph{Raspberry PI}. Además cuentan con el robot simulado en 
el simulador Gazebo. Lo que dota al estudiante de flexibilidad a la hora de realizar los ejercicios
fuera del horario lectivo. En la figura a continuación se muestran una imagen de la infraestructura
hardware (PiBot).\\
...
\begin{itemize}
	...
	\url{www.appear.in} que es un servicio de videollamada similar a Skype pero en Web.
	 ...
	 Utiliza el motor WebGL anteriormente mencionado para el renderizado de las imágenes 3D.
\end{itemize}

A continuación se muestran imágenes que presentan aplicaciones web que utilizan algunas de estas 
mejoras del nuevo estándar HTML 5.
...
~\ref{web-evolution} se muestra de manera esquemática y muy resumida la evolución de la web.\\

\begin{figure}
	\centering
	\includegraphics[width=10cm, height=7cm]{img/web-comparativa.jpg}
	\caption{Esquema que muestra las nuevas relaciones que se establecen según evoluciona la web entre
...
Mi trabajo fin de grado consiste en crear la base de una herramienta educativa (WebSim) para 
...
El simulador ha de soportar la infraestructura hardware utilizada actualmente por JdeRobot-Kids,
...
\begin{itemize}
	...
	\item \emph{Cámara}: una minicámara, ésto le da funcionalidad muy 
...
Además existen dos objetivos más, la creación de dos aplicaciones web para el uso de WebSim a través
de dos editores de código distintos en los cuales los alumnos que usen el simulador podrán resolver
una serie de ejercicios educativos propuestos por JdeRobot-Kids.


\section{Metodología}
\label{sec:metodologia}

Para el desarrollo del proyecto se ha seguido la metodología \emph{Agile}, es una forma de realizar
...
Como se ha comentado en la subsección anterior se ha llevado una metodología en \emph{sprints}, a 
continuación se muestran dos imágenes con los intervalos temporales que han llevado cada uno de los
\emph{sprints}.
...
Como se ve en las imágenes algunos de los \emph{sprints} han ocupado más de una reunión semanal. En
...
necesidad de aprender diferentes tecnologías como son AFRAME, jQuery,
...
para este proyecto se ha utilizado la plataforma de control de versiones \emph{GitHub}. Github es 
...
características lo hacen muy potente y admitido por la comunidad del desarrollo de software en general.\\

Como última característica pero no menos importante Github permite visualizar de forma gráfica la
...
\begin{figure}[h]
	...
	\caption{Gráfico de actividad del repositorio de \emph{AFRAME} en Github.}
...
\emph{pull request} a modo de parche para la incidencia lo que permite que el desarrollo quede muy 
...
proyecto. Estas herramientas han sido elegidas debido a que eran las que mejor se ajustaban a las 
necesidades del proyecto.\\

Algunas de las herramientas utilizadas como HTML, y JavaScript se han elegido debido a que son los
...
\begin{itemize}
	\item Lenguaje de \emph{alto nivel}, esto quiere decir que su 
	...
	\item Lenguaje basado en \emph{objetos}, esto es una estructura
	...
	\begin{figure}[h]
		...
		objeto \emph{coche} del que heredan los 3 de la parte
		derecha de la imagen, los 3 coches tienen en común que son
		del objeto coche pero se diferencian en \emph{marca} y
		\emph{color.}}
...
\begin{itemize}
	...
	\begin{figure}[h]
  		...
  		\end{minipage}
  		\caption{Comparativa de sintaxis de declaración de un objeto antes y después del estándar ES6.}
  		\label{fig:class-def}
	\end{figure}
...
\begin{itemize}
	...
	\item \emph{AFRAME} Es un entorno de creación de escenas de 
...
AFRAME está creado en el lenguaje JavaScript la elección de este lenguaje adquiere importancia por 
sí misma.
...
llamados \emph{\emph{hyperlinks}} que permiten la navegación entre distintos recursos.
...
\emph{\emph{AFRAME}} es un entornos web para la construcción de escenas de realidad
...
de la primitiva \emph{a-entity} que, equiparandolo con HTML convencional, equivaldría con la
...
AFRAME se basa en el entorno \emph{\emph{three.js}} y provee una estructura reutilizable de entidad-
...
\begin{itemize}
	\item \emph{Entidad:} se representa en AFRAME mediante la etiqueta \emph{a-entity}.
	\item \emph{Componentes:} se representa en AFRAME como atributos de la etiqueta HTML. Estos 
	componentes son objetos que contienen un esquema, manejadores y métodos. Éstos se registran mediante
	el método \emph{AFRAME.registerComponent(nombre, definición);}. A continuación se muestra un 
	...
	\emph{AFRAME.registerSystem(nombre, definición)}.
\end{itemize}
...
\emph{AFRAME} ofrece la posibilidad de cargar modelos 3D más sofisticados en los formatos 
...
\emph{Don McCurdy}, se puede localizar en \url{https://github.com/donmccurdy/aframe-extras/blob/master/src/loaders/animation-mixer.js}.

\subsection{Herramientas de Desarrollo}
\label{subsec:devtools}

\emph{AFRAME} como hemos comentado se construye sobre JavaScript y HTML por tanto utiliza las mismas
...
\emph{\emph{ACE Editor}} es un editor de código embebido creado en \emph{JavaScript}, implementa las 
...
de nuestra aplicación, simplemente hace falta una etiqueta \emph{script} y un pequeño código para
...
\emph{canvas} y no depende de librerías externas. La característica más destacable de el editor es que 
...
\emph{\emph{WebPack}} que es una herramienta de empaquetado de aplicaciones lo que permite generar un
...
En este capítulo se muestra la estructura diseñada para el simulador robótico web y se detallan sus partes
...
\begin{figure}[h]
  \centering
  \includegraphics[width=12cm, height=6cm]{img/Arquitectura-websim.PNG}
  \caption{Estructura de WebSim.}
  \label{fig:arquitectura}
\end{figure}

Como se comentó en el capítulo ~\ref{chap:intro}, el objetivo del presente proyecto es crear 
una aplicación para simular robots en el entorno \emph{AFRAME} permitiendo conectar un 
editor de código para ejecutar instrucciones con el robot simulado. El primer componente de esta aplicación
es el propio simulador robótico. En la figura ~\ref{fig:arquitectura} se muestra el esquema que sigue el simulador WebSim. A continuación
...
\begin{itemize}
	\item Registrar los tres componentes constituyentes de \emph{AFRAME}. Para la simulación del robot en el 
	entorno ha sido necesario crear tres componentes 
	llamados \emph{followBody, spectatorComponent, intersectionHandler} que se encargan simular una cámara 
	contenida en el robot, manejar los eventos de intersección de láseres en el robot simulado y 
	anclar al cuerpo del robot simulado un elemento respectivamente. La figura 
	...
	\item Ofrece la conexión simplificada con el robot simulado en el entorno AFRAME , 
	\emph{WebSim} ofrece un interfaz en lenguaje JavaScript llamada \emph{HAL API} 
	\emph{(Hardware Abstraction Layer)} lo que simplifica mucho el uso del robot creado en AFRAME. Se le 
	...
	tener que comunicarse directamente con el motor \emph{AFRAME} ya que de eso se encarga \emph{WebSim}.
	...
	Y segundo, evita la creación de dos instancias del mismo objeto que llevaría a una incorrecta 
	ejecución.
	
	\item Controlar la ejecución del \emph{mundo}, es decir, permite arrancar o pausar el robot. 
	...
	\item Permite la conexión de software externo mediante intercambio de mensajes. De este modo el software 
	...
	simulados y acceder a los actuadores simulados.
...
Algunos ejemplos de ejercicios realizados son sigue línea, seguimiento de objeto mediante filtrado
de colores y evitar obstáculos mediante el sensor de distancias, como se describirá en el capítulo 
~\ref{chap:robotica-educativa}.
...
En esta sección se explica como se ha creado la simulación 3D de nuestra escena con el entorno 
\emph{AFRAME}. \\
...
~\ref{fig:escena-simulador} se muestra el esquema HTML necesario para generar nuestra escena junto con nuestro 
robot. Ahora se describirán cada una de las partes del mundo simulado, es un conjunto de elementos con 
atributos y \emph{AFRAME} se encarga del funcionamiento tridimensional.\\
...
\begin{figure}[h]
	...
	\caption{Etiqueta \emph{a-scene}, es la etiqueta principal del entorno \emph{AFRAME}.}
	\label{fig:a-scene}
\end{figure}

Como se ha comentado en el capítulo ~\ref{chap:herramientas} la etiqueta \emph{a-scene} 
(Figura ~\ref{fig:a-scene}) es la etiqueta principal utilizada en \emph{AFRAME} para generar la visualización. 
Como queremos una escena simple le hemos dado un color de fondo gris y hemos activado las físicas y 
mediciones. Las físicas permite hacer uso del motor de físicas en el paquete \emph{aframe-physics}
en el cual existe la gravedad, rozamiento y otros tipos de fuerzas. La etiqueta \emph{stats}  
...
\begin{figure}[h]
	...
	\caption{\emph{a-assets} es la etiqueta usada por \emph{AFRAME} para la gestión de texturas y 
	recursos externos.}
	\label{fig:a-assets}
\end{figure}

Las etiquetas \emph{a-assets} (Figura ~\ref{fig:a-assets}) son un modo de \emph{AFRAME} de gestionar de manera
...
\begin{itemize}
	...
	\item \emph{Loaded}: Indica que el archivo ha cargado correctamente.
\end{itemize}

\subsection{Cuerpo del robot}
\label{subsec:cuerpo}

Como se ha explicado en el capítulo ~\ref{chap:herramientas}, el 'objeto' básico en una escena en 
\emph{AFRAME} es \emph{a-entity} que es un objeto vacío al que le podemos conectar y dar
la funcionalidad que requiramos. Haciendo uso de estas etiquetas se ha simulado el robot \emph{Pibot}, nuestro 
robot se compone de ruedas con motores, sensores y cámara. La Figura ~\ref{fig:tags-pibot} muestra las 
...
\begin{figure}[h]
	...
	\caption{Estructura de etiquetas utilizada para la simulación del robot en el entorno \emph{AFRAME}.}
...
\begin{figure}[H]
	...
	\caption{Robot simulado en la escena, se resalta la cámara integrada en él.}
...
que dota de físicas al elemento con la particularidad que el elemento puede ser movido
por otros. 
...
la figura ~\ref{fig:raycasters}. Estos sensores representan a sensores reales de ultrasonidos o láser.
La carga de estos elementos se hace desde WebSim, es necesario hacerlo así para poder modificarlos 
dinámicamente y añadir más o menos sensores de distancia al robot. Se han utilizado un array de sensores de 
...
\begin{figure}[h]
	...
	\caption{Etiqueta vacía utilizada para encapsular los sensores de ultrasonido.}
...
Por último tenemos la cámara, como vemos en la Figura ~\ref{fig:pibot-simulado} es un elemento hijo 
encapsulado dentro de una entidad. Esto se hace así debido a las recomendaciones de \emph{AFRAME} que indican 
que para modificar la posición de la cámara incluyamos esta dentro de una entidad. La etiqueta cámara cuenta 
con atributos entre los que destacaremos dos \emph{spectator y wasd-controls-enabled}.
...
En la Figura ~\ref{fig:pibot-simulado} se muestra el robot simulado y una muestra
del foco de visión que está tomando la cámara del robot en la escena, es decir, lo que está viendo el robot.
...
\begin{figure}[h]
	...
	\caption{Etiquetas utilizadas para crear el resto de elementos de la escena del simulador \emph{WebSim}.}
	\label{fig:etiquetas-escena}
\end{figure}

La etiqueta \emph{a-sphere} declara la existencia de una esfera en nuestra escena, los atributos 
...
El robot consta de varios sensores y actuadores, simulados por \emph{AFRAME} y a los
cuales se puede acceder desde código externo al simulador a través de un
interfaz de programación llamado \emph{HAL-API (Hardware Abstraction Layer)}. 
También materializan la simulación del robot (por ejemplo el movimiento) manejando la representación web del
robot en \emph{AFRAME}.\\


Por ejemplo los sensores implementados en el robot recogen una serie de datos que han de ser guardados 
de manera interna en el robot para que el usuario pueda dar uso de éstos y programar una lógica.
Para ello se crean una serie de métodos que se conectan con el robot simulado en
...
en otras variables internas del robot, lo que permite hacer uso de estos datos de manera asíncrona.
...
parámetro de entrada un \emph{string} con el identificador de la etiqueta HTML (etiqueta del entorno AFRAME) 
en la cual tenemos el robot simulado. Gracias a esto se puede hacer uso desde \emph{JavaScript} de las 
propiedades que ofrece el entorno AFRAME, como la posición en la escena, añadir o eliminar elementos 
en la escena, obtener la imagen de la cámara, etc. \\

Además de acceder al robot simulado de AFRAME el constructor inicia una serie de 
...
\begin{itemize}
	...
	objeto robot y el robot simulado en AFRAME y permite acceder a los distintos
	...
	a la hora de programar la lógica del robot es necesario conocer la velocidad actual, esto nos 
	...
	velocidlineal, de manera simplificada sería la velocidad a la que se mueve el robot en la dirección en la 
...
de manera autónoma hasta nueva orden.\\ 
...
\begin{itemize}
	\item Obtenemos la orientación actual, esto se hace debido a que, como el robot se encuentra en un 
	...
	vector de vista (hacia donde mira el robot). Para calcular dicha posición es necesaria la orientación,
	...
	\begin{figure}[h]
		...
		\caption{Cálculo de la nueva posición en función del ángulo de orientación y la velocidad lineal.}
	\end{figure}
	...
	la función se ejecute así misma constantemente y no tener que estar enviando continuamente 
	instrucciones de velocidad lo que simplifica el código.
\end{itemize}


Gracias a esta última instrucción nativa de \emph{JavaScript} podemos crear una función iterativa. Con ello
el modo de usar los motores se simplifica, el usuario únicamente llama a la función \emph{setV}
...
cuál es la velocidad configurada por el usuario. Esto permite simular una hebra dentro del navegador web y
...
Finalmente se muestra una tabla que resume las funciones que se ofrecen en el \emph{HAL API} para usar los 
motores en el robot simulado.
...
\endhead
%
.setV(velLineal) & \begin{tabular}[c]{@{}c@{}}Mueve hacia delante o atrás el robot.\\ INPUT:\\ - velLineal: numero con la velocidad lineal(m/s).\end{tabular} \\ \hline
.setW(velAngular) & \begin{tabular}[c]{@{}c@{}}Hace girar al robot.\\ INPUT:\\ - velAngular: numero con la velocidad angular (m/s).\end{tabular} \\ \hline
...
\subsection{Sensor de distancia a obstáculos}
...
ha sido simulado mediante el atributo \emph{raycaster} existente en AFRAME que se asemeja a un láser y
...
\begin{itemize}
	\item Componente AFRAME \emph{followBody}: Este componente tiene una función muy simple, anclar 
	...
	\item Componente AFRAME \emph{intersectionHandler}: Este componente es el más importante ya que
	...
	\emph{raycaster} que lanza el evento. Esto se lleva a cabo para detectar qué \emph{raycaster}
	...
	atributos como la distancia máxima a la que se detectarán intersecciones. Además se le añade los
	...
	es decir, para el sensor de obstáculos.
	...
	\item \emph{getDistance y getDistances}: Estas dos funciones son las que se proporcionan al  
...
\emph{startRaycasters} y esta se encarga de configurar todo llamando a las funciones que se han mencionado
anteriormente.\\
...
El sensor de infrarrojos se ha implementado mediante el uso de la cámara (Figura ~\ref{fig:pibot-simulado}) integrada en el cuerpo del robot 
simulado por simplicidad y por mejora de rendimiento, lo que se hace internamente es recortar la imagen de la 
cámara hasta quedarnos con los píxeles que se encuentran más abajo en la imagen, el ancho de la imagen se 
...
entrada un color como \emph{string} y accede a la variable anteriormente mencionada 
...
\begin{itemize}
	\item \emph{0}: Los dos sensores infrarrojos están detectando la línea.
	
	\item \emph{1}: Únicamente el sensor infrarrojo de la izquierda detecta la línea.
	
	\item \emph{2}: Únicamente el sensor infrarrojo de la derecha detecta la línea.
	
	\item \emph{3}: Ninguno de los sensores detecta la línea.
\end{itemize}

Esta simplificación se debe a la implementación existente en el robot real, un objetivo en el futuro
es que el mismo programa creado en el robot se pueda exportar al robot real y funcione de la misma
manera, por tanto convenía mantener compatibilidad para que las aplicaciones funcionen sin que sean 
necesarios grandes cambios tanto en el robot real como en el simulado.\\
...
.readIR(color) & \begin{tabular}[c]{@{}c@{}}Obtiene valores entre 0-3 acordes a dos \\ el uso de sensores infrarrojos para un \\ color de línea pasado como parámetro\\ INPUT:\\ -color: color a filtrar en la imagen como string\end{tabular} \\ \hline

\end{longtable}

\subsection{Sensores de odometría}

Los sensores de odometría se encargan de obtener la posición del vehículo durante la navegación, para la 
simulación de estos sensores no ha sido necesario ningún componente extra ni entidad, se ha hecho uso del sistema de coordenadas y rotación de \emph{AFRAME} que permite la obtención de estos datos a través de su API
JavaScript. De esta manera hemos podido desarrollar dos funciones que simplifican el acceso a los datos, estas
funciones se describen en la siguiente tabla.
...
Cabe comentar el sistema de coordenadas del que hace uso \emph{AFRAME} para así entender en mejor manera
los datos que se obtienen. La figura ~\ref{fig:aframe-axis} muestra los ejes utilizados.

\begin{figure}[h]
	\centering
	\includegraphics[width=7cm, height=7cm]{img/axis-aframe.png}
	\caption{Sistema de ejes utilizado en las escenas del entorno \emph{AFRAME}}
	\label{fig:aframe-axis}
\end{figure}
...
\begin{itemize}
	...
	\item \emph{getImage}: Esta función la invocará el usuario final para obtener los datos de la cámara
	y con ello materializar la lógica que se necesite.
	
\end{itemize}

Además de estos métodos 'crudos' se ofrecen una serie de métodos algo más elaborados, 'cocinados', 
...
\emph{boolean} que indica si el robot ya tiene un código ejecutando en ese momento.\\

Si el robot está ejecutando lo que hace \emph{WebSim} es parar el robot, es decir, ponerle la velocidad a 0 
tanto lineal como angular y parar el hilo de ejecución del código que ha programado el usuario. De esta manera
implementamos la 'pausa académica' que permite parar el robot, modificar una parte del código y volver a
ejecutar el algoritmo sin tener que refrescar la página y rehacer el código desde el principio.\\

Si el robot no tiene ningún código ejecutando entonces \emph{WebSim} permite la ejecución del código mediante el 
uso de la instrucción nativa de JavaScript \emph{eval()}. Es conocido que esta instrucción tiene una 
vulnerabilidad y es que permite la inyección de código maligno. Se ha estudiado esta vulnerabilidad y en
el contexto que nos atañe no habría problema ya que es una aplicación totalmente local y todo código
malicioso que se quisiese inyectar se inyectaría únicamente en la maquina local.


\section{Conexiones de \emph{WebSim} con software externo}
\label{sec:conexion}

Como se ha señalado en la sección ~\ref{sec:arquitectura}, \emph{WebSim} permite la conexión con 
software externo para manejar el robot simulado recogiendo medidas sensoriales y enviando órdenes a los 
...
y ejecutar. Esto permite por tanto la conexión con editores embebidos en la página e incluso conectar 
\emph{WebSim} con una aplicación externa al navegador conectándose a través de \emph{Websockets}.
...
minificar (reducir) el código de la aplicación una vez empaquetado a modo de optimizar el rendimiento para 
...
dependencia. Desde el archivo \emph{websim.js} se importan las dependencias de
...
\begin{figure}[h]
	\centering
	\includegraphics[width=5cm, height=9cm]{img/webpack.PNG}
...
tenemos dos nombres \emph{websim.bundle.js y editor.bundle.js}.
...
módulos, por ejemplo si hacemos uso de la instrucción \emph{import 'aframe'} lo que hace es buscar la
dependencia con el nombre 'aframe' y extensión declarada en la configuración.

\begin{figure}[h]
	\centering
	\includegraphics[width=5cm, height=3cm]{img/devserver-webpack.PNG}
	\caption{Declaración de la configuración del servidor estático para desarrollo que \emph{WebPack}
	permite usar.}
	\label{fig:devserver}
\end{figure}
...
las extensiones de los ficheros que vamos a usar, en este caso \emph{.js y .jsx (React)}. La extensión '.jsx'
no va a ser utilizada en este caso.

Esto nos permite compilar y transformar archivos en \emph{TypeScript} por ejemplo a JavaScript para ser 
...
Una vez ejecutadas cualquiera de las dos opciones abrimos el navegador (Firefox) e ingresamos la siguiente URL.
...
Como se ha comentado en el capítulo ~\ref{chap:intro} del presente documento, los usos del simulador irán
orientados a la robótica educativa. Se han creado dos aplicaciones de manera que los usuarios aprenderán a 
programar la lógica del robot usando una simplificación de acceso a los sensores y actuadores del robot simulado.\\

Se han preparado las aplicaciones para desarrollar una serie de ejercicios ya establecidos previamente al
...
acceder a la siguiente URL. Es necesario acceder con el navegador \emph{Firefox}.\\
...
La primera de las dos aplicaciones anteriormente mencionadas es una aplicación web que hace uso del simulador
\emph{WebSim} y el editor ACE Editor en la cual el alumno puede hacer uso del HAL API del robot simulado en
lenguaje JavaScript para programar la lógica del robot y resolver un ejercicio propuesto.\\

El módulo del editor se encarga principalmente de crear los manejadores de eventos para los eventos de los
botones embebidos en la página web, configurar el renderizado del editor y la obtención de código escrito por 
el usuario.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=11cm, height=8cm]{img/Websim-JS.PNG}
	\caption{Arquitectura de la aplicación que hace uso del editor ACE y WebSim.}
	\label{fig:websim-js}
\end{figure}

Como se ve en la figura la arquitectura tiene dos módulos, uno para el editor y el módulo de WebSim lo que 
dota de flexibilidad a la hora del desarrollo permitiendo tener distintos editores distintos o interfaces de
entrada distintas.
...
\begin{itemize}
	...
	para obtener el código que el usuario ha escrito en el editor embebido y lo almacena en una variable.
...
\section{Ejercicios con \emph{JavaScript}}
\label{sec:ejercicios-js}

Como se ha comentado en el inicio del capítulo existen 3 ejercicios predefinidos que utiliza la 
plataforma \emph{JdeRobot Kids} para enseñar a los alumnos programación de robots y visión artificial.
...
A continuación se muestra una lista de enlaces a los ejercicios propuestos por \emph{JdeRobot Kids}
resueltos:
\begin{itemize}
...
que se pueden crear distintas paginas para plantear tantos ejercicios como se quiera con el mismo robot tal
y como se muestra en la figura ~\ref{fig:interfaz-js} en la cual tenemos una escena 3D generada para la 
plataforma \emph{JdeRobot Kids} en la cual tenemos un mayor número de obstáculos con el fin de aprender a 
programar la lógica para que el robot no se choque con los elementos.\\

\begin{figure}[H]
	\centering
	\includegraphics[width=12cm, height=6cm]{img/interfaz-js.PNG}
	\caption{Interfaz de usuario de WebSim + editor JavaScript (Ace Editor) usado en \emph{JdeRobot Kids}.}
	\label{fig:interfaz-js}
\end{figure}



\section{Programando con \emph{Scratch}}
...
los ejercicios planteados en \emph{JdeRobot Kids} mediante el uso de bloques visuales con 
\emph{Blockly}.\\
...
interfaz de tipo \emph{Plug and Play} en la que se conectan los bloques unos con otros para
generar código.

\begin{figure}[h]
	\centering
	\includegraphics[width=12cm, height=6cm]{img/interfaz-blockly.PNG}
...
En la figura ~\ref{fig:websim-blockly} se muestra la arquitectura usada para la construcción de la aplicación
web. Como se puede apreciar tanto esta como la arquitectura en la figura ~\ref{fig:websim-js} son similares, 
los únicos módulos que cambian son los referentes al editor que cada editor necesitará una configuración 
diferente en función de sus características.\\

\begin{figure}[h]
	\centering
	\includegraphics[width=11cm, height=8cm]{img/Websim-Blockly.PNG}
	\caption{Arquitectura de la aplicación web que hace uso de Websim + Blockly.}
	\label{fig:websim-blockly}
\end{figure}

Al estar usando Blockly necesitamos crear los bloques personalizados que, al ser usados por el usuario, se 
traducirán a las instrucciones JavaScript que hacen uso del HAL API que ofrece WebSim.\\

El flujo de ejecución de código es el mismo que el mencionado en la sección ~\ref{sec:ejercicios-js} pero 
modificando el módulo del editor a los necesarios para configurar Blockly y traducir los bloques al lenguaje
JavaScript.

\section{Ejercicios con \emph{Scratch}}
\label{sec:ejercicios-scratch}

Los ejercicios propuestos son similares a los mencionados en la sección ~\ref{sec:ejercicios-js} a excepción
del ejercicio de manejar el robot mediante el teclado. \\

La diferencia en los ejercicios reside principalmente en la escena que se va a usar, los métodos necesarios
para su resolución y la plantilla de bloques visuales que se proporciona al usuario.\\

En la figura ~\ref{fig:interfaz-blockly} se muestra la escena necesaria para la propuesta del ejercicio en el
cual se pide al usuario programar la lógica para perseguir la esfera de color verde. Se menciona ésta en 
concreto debido a que para que la esfera se mueva necesitamos importar un script en el navegador en el cual 
dotemos de movimiento aleatorio a la esfera.\\

Estos ejercicios nos permiten validar el desarrollo del proyecto ya que nos permiten comprobar la estabilidad
y flexibilidad del simulador y más concretamente del HAL API desarrollada para el robot.
...
contrario, el servidor utilizado es un servidor estático que ofrece la plataforma de \emph{Github}.\\
