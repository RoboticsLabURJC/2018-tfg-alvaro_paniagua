...
puede acceder a la siguiente URL. En ellas se han realizado una serie de ejercicios típicos de enseñanza de
robótica ya establecidos previamente al
proyecto. Es necesario remarcar que \emph{A-Frame} está soportado en los siguientes
navegadores y en ellos funcionan correctamente las dos aplicaciones web que aquí se describen. Los navegadores móviles no tienen soporte VR oficial por lo tanto el rendimiento será notablemente peor que en el resto.

Los navegadores soportados son: Firefox 55+ para Windows, Supermedium, Chromium experimental, Navegador Oculus (GearVR), 
Samsung Internet (GearVR), Microsoft Edge, (Móvil) Safari para iOS, (Móvil) Chrome para Android, (Móvil) Firefox para iOS,
(Móvil) Samsung Internet, (Móvil) Navegador UC. Pese a estar soportado en varios navegadores se recomienda su uso en Firefox 
...
entrada distintas. El módulo del editor se encarga principalmente de crear los manejadores de eventos para los eventos de los
...
Existen 3 ejercicios típicos que se utilizan en la 
...
Trata de crear la lógica interna del robot con el objetivo de seguir una línea que 
...
El ejercicio consiste en programar la lógica del robot utilizando los sensores de obstáculos y los motores para que el robot sea capaz de moverse de manera autónoma en el entorno evitando los
distintos objetos de la escena. Una aplicación práctica de esta lógica es el conocido aspirador autónomo \emph{Roomba}.
...
uso del método del HAL API del robot simulado \emph{getObjectColor} y \emph{getObjectColorRGB} que ofrecen 
...
y el robot se acercan o se alejan (si el área es mayor se están acercando y si es menor se están alejando).
...
Como se ve en la Figura ~\ref{fig:fball-code-js} se configuran una velocidad de giro en función de dónde
se encuentra el centro de la esfera en la cámara del robot. El algoritmo mostrado es un algoritmo muy 
...
Haciendo uso del mismo núcleo de \emph{WebSim} se ha creado una pequeña aplicación web para la resolución de
los ejercicios planteados en \emph{JdeRobot Kids} mediante el uso de bloques visuales con 
\emph{Blockly}.\\

Esta aplicación web se ha creado para una toma de contacto con la lógica de programación de robots para 
...
modificando el módulo del editor de modo que incluya los pasos necesarios para configurar Blockly y traducir los bloques al 
lenguaje JavaScript.
...
La diferencia entre los tres ejercicios reside principalmente en la escena que se va a usar, los métodos necesarios
...
para hacer uso de el método del \emph{HAL API}. El código resultante es el de la Figura ~\ref{fig:fl-scratch-code}:
...
cabo los puntos establecidos. El principal punto era crear un simulador en lado cliente y se ha diseñado, desarrollado y 
...
se ha podido intuir a lo largo del documento no existe un servidor con una gran funcionalidad y coste computacional, al 
...
interesantes que se podrán ir implementando a posteriori como la integración con Electron para crear una aplicación de 
escritorio o hacer uso de otros lenguajes de programación como Python.
...
Herramientas de desarrollo de Google Blockly:
