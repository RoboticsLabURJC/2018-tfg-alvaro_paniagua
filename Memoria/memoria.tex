...
\emph{\emph{A-FRAME}} es un entornos web para la construcción de escenas de realidad
...
A-FRAME además está soportado en los siguientes escenarios:
\begin{itemize}
...
Crear escenas de realidad virtual de manera muy simple, como se ve en la figura~\ref{fig:A-FRAME-scene}
...
A-FRAME permite además crear nuestras propias primitivas lo que nos permite seguir el principio de 
...
\centering
	...
	\caption{La siguiente figura muestra etiquetas que serían el equivalente en A-FRAME a un componente,
...
Además como hemos comentado A-FRAME tiene dos tipos de implementaciones que le dan características 
...
\begin{itemize}
	...
	\begin{figure}[h]
		...
		\caption{Ejemplo de acceso a un elemento de A-FRAME, como se aprecia se hace de la misma
...
\emph{Don McCurdy}, se puede localizar en \url{https://github.com/donmccurdy/A-FRAME-extras/blob/master/src/loaders/animation-mixer.js}.

\subsection{Herramientas de Desarrollo}
\label{subsec:devtools}

\emph{A-FRAME} como hemos comentado se construye sobre JavaScript y HTML por tanto utiliza las mismas
...
la intención es poder conectar distintos tipos de entradas a WebSim como puede ser un editor externo de 
código en JavaScript, un editor de código en el lenguaje visual Blockly, una aplicación externa vía 
...
\begin{itemize}
	...
	llamados \emph{followBody, spectatorComponent, intersectionHandler} que se encargan de simular una cámara 
	contenida en el robot, manejar los eventos de intersección de láseres en el robot simulado y 
	anclar al cuerpo del robot simulado un elemento respectivamente.
	
	\item Ofrece la interfaz de programación en JavaScript para manejar el robot simulado , 
	ofrece un interfaz en lenguaje JavaScript, llamada \emph{HAL API} 
	\emph{(Hardware Abstraction Layer)} lo que simplifica mucho el uso del robot creado en A-FRAME. Se le 
	...
	simulados y acceder a los actuadores simulados. Es un segundo interfaz de programación del robot simulado,
...
\begin{figure}[H]
	\centering
	\includegraphics[width=17cm, height=10cm]{img/aframe-sim.PNG}
	\includegraphics[width=17cm, height=10cm]{img/aframe-sim-2.PNG}
...
\begin{itemize}
	...
	HTML lo que permite mostrar lo que está viendo el robot en tiempo real.
...
declara el tamaño del plano, la textura a cargar en el plano que en este caso será un circuito en 
...
ultrasonido que se explicarán más detalladamente en sus correspondientes subsecciones a continuación.

La figura ~\ref{fig:constructor-robot} muestra el código del constructor del robot. Como se ve, la variable \emph{this.understandedColors} es una simplificación de los filtros a aplicar en 
...
Gracias a esta última instrucción nativa de \emph{JavaScript} se puede crear una función iterativa. Con ello
...
\begin{table}[h!]
  \begin{center}
    \caption{Métodos (API) de los motores del robot.}
    \vspace{0.5cm}
...
\begin{itemize}
	...
	ocurre se llama a la función \emph{updateDistance} y cuando ocurre el evento \emph{intersection-cleared-'id del raycaster'} se llama a la función \emph{eraseDistance}. Ambas funciones se explican en sus
...
La Tabla ~\ref{tab:tabla-raycasters} muestra las funciones que se ofrece al usuario para hacer uso de 
...
\begin{itemize}
	\item \emph{0}: Los dos sensores ficticios infrarrojos están detectando la línea.
...
Este formato de salida se debe a la implementación existente en el robot real para ser compatible con ella.
...
\begin{table}[H]
  \begin{center}
    \caption{Métodos (API) de los sensores infrarrojos del robot simulado.}
    \vspace{0.5cm}
    \label{tab:tabla-ir}
    \begin{tabular}{|c|c|} % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      \textbf{Método} & \textbf{Descripción}\\
...
Cabe comentar el sistema de coordenadas del que hace uso \emph{A-FRAME} para entender 
los datos que se obtienen en estas funciones del HAL API. La Figura ~\ref{fig:A-FRAME-axis} muestra los ejes utilizados.
...
\emph{boolean} que indica si el robot ya se está usando, si ya tiene un código ejecutando en ese momento.\\
...
y ejecutar comprobando previamente como se ha comentado en secciones anteriores si el robot simulado
...
\emph{Websockets} como puede ser ROS o otro tipo de protocolos de comunicación como ICE.

Ambas interfaces (ROS y ICE) se ha programado en WebSim pero no forman parte del núcleo de este
...
a un código que \emph{WebSim} interpretará y ejecutará en el robot simulado.
...
cargar la página por parte de un usuario. Se ha programado en \emph{WebSim} pero no forma parte del núcleo de este
Trabajo Fin de Grado, por lo que apenas de describe un esbozo de este
bloque software del simulador robótico.\\
...
Una vez ejecutadas cualquiera de las dos opciones abrimos el navegador, se recomienda Firefox debido a que  A-FRAME tiene compatibilidad total con éste e ingresamos la siguiente URL.
...
Como se ha comentado en el capítulo ~\ref{chap:intro} del presente documento, los usos del simulador propuesto
van orientados a la robótica educativa. Se han creado dos aplicaciones web docentes de manera que los usuarios aprenderán a programar la lógica del robot usando el acceso simple a los sensores y actuadores del robot simulado que proporciona \emph{WebSim}.\\

Se han preparado dos aplicaciones web para desarrollar una serie de ejercicios ya establecidos previamente al
proyecto.

Para el uso de ambas aplicaciones web podemos instalarlas en local que se explicará a continuación o bien se 
puede acceder a la siguiente URL. Es necesario remarcar que \emph{A-Frame} está soportado en los siguientes
navegadores. Los navegadores móviles no tienen soporte VR oficial por lo tanto el rendimiento será notablemente peor que en el resto.

\begin{itemize}
	\item Firefox 55+ para Windows.
	\item Supermedium.
	\item Chromium experimental.
	\item Navegador Oculus (GearVR).
	\item Samsung Internet (GearVR).
	\item Microsoft Edge.
	\item (Móvil) Safari para iOS.
	\item (Móvil) Chrome para Android.
	\item (Móvil) Firefox para iOS.
	\item (Móvil) Samsung Internet.
	\item (Móvil) Navegador UC.
\end{itemize}

Pese a estar soportado en varios navegadores se recomienda su uso en Firefox siempre que así sea posible.
...
\emph{WebSim} y el editor ACE Editor el cual el alumno puede utilizar el HAL API del robot simulado en
...
\begin{figure}[h]
	\centering
	\includegraphics[width=13cm, height=9cm]{img/Websim-JS.PNG}
...
La figura ~\ref{fig:js-ui} muestra la interfaz de usuario de la aplicación web que hace uso de \emph{WebSim}
...
Como lo que tenemos es una aplicación web con un editor de código \emph{JavaScript} se pueden incluso crear
...
\subsection{Ejercicio \emph{Sigue Lineas}}

El siguiente ejercicio trata de crear la lógica interna del robot con el objetivo de seguir una línea que 
describe un circuito haciendo uso de los sensores infrarrojos en el robot simulado. Para ello se hará uso del
...
~\ref{fig:fl-execution} se muestran 3 fotogramas con la ejecución del código mostrado en la Figura 
~\ref{fig:fl-code-js}.
...
El ejercicio consiste en programar la lógica del robot haciendo uso de los sensores de obstáculos del robot 
simulado y los motores para que el robot sea capaz de moverse de manera autónoma en el entorno evitando los
distintos objetos de la escena. Una aplicación práctica de esta lógica es el conocido aspirador \emph{Roomba}.

\begin{figure}[h]
	\centering
	\includegraphics[width=10cm, height=7cm]{img/hitturn-code-js.PNG}
	\caption{Lógica programada en la aplicación web para resolver el ejercicio \emph{Evitar Obstáculos}.}
	\label{fig:hitturn-code-js}
\end{figure}

La lógica es básica, el robot avanza hasta que uno de los sensores de obstáculos del robot simulado detecta 
un objeto cuya distancia respecto al robot se guarda en la variable \emph{distancias}. Se comprueba si las 
distancias guardadas en la variable mencionada son menores a un umbral que establece el usuario, en este caso
es 1 metro. Si la condición se cumple el robot deja de avanzar para girar hasta que se haya evitado el 
...
El ejercicio \emph{Sigue Objeto} es una mejora del ejercicio \emph{Sigue Linea} en el cual se hace uso del 
sensor más potente que posee el robot simulado, la cámara. Es un ejercicio que permite poner en práctica 
conocimientos sobre visión artificial y tratamiento de imagen. Para la resolución del ejercicio se hace uso
del método del HAL API del robot simulado \emph{getObjectColor} y \emph{getObjectColorRGB} que ofrecen 
las coordenadas en la imagen del centro del objeto detectado con el color que el usuario pasa como parámetro
...
se acerca hacia nosotros o se aleja (si el área es mayor se está acercando y si es menor se está alejando).
...
Como se ve en la Figura ~\ref{fig:fball-code-js} configuramos una velocidad de giro en función de dónde
se encuentra el centro de la esfera en la cámara del robot. El algoritmo mostrado es un algoritmo lógico muy 
básico, este ejercicio permite además un algoritmo con un enfoque matemático en el cual la velocidad de giro
variará con la posición del centro de la esfera en la cámara del robot simulado siguiendo la ecuación de una
recta en la cual la velocidad de giro será mayor en función de si la esfera se encuentra en los extremos de la
imagen. Este algoritmo se adapta mejor al caso en el cuál la esfera en la escena se encuentre en movimiento.
...
Como se aprecia, este ejercicio tiene un gran peso ya que se puede incrementar la dificultad añadiendo 
...
Haciendo uso del mismo core de \emph{WebSim} se ha creado una pequeña aplicación web para la resolución de
...
\begin{figure}[H]
	...
	\centering
	\includegraphics[width=13cm, height=9cm]{img/Websim-Blockly.PNG}
...
traducirán a las instrucciones JavaScript que hacen uso del HAL API que ofrece WebSim. Estos bloques necesitan
tres 'piezas' para ser utilizados correctamente:

\begin{itemize}
	\item JSON de configuración, en este se configuran los parámetros de entrada, el texto en el bloque, el 
	color del bloque y todos los atributos necesarios. Por simplicidad se ha usado las herramientas de 
	desarrollo que ofrece Google en las cuales te permite generar un bloque personalizado a partir de otros 
	bloques. 
	...
	de dicha función.
		\begin{figure}[H]
			...
			\caption{Función que muestra el bloque personalizado de \emph{Blockly} en el menú del editor
			embebido.}
	...
	\begin{figure}[H]
			...
			\caption{Función que genera el código JavaScript del bloque personalizado de \emph{Blockly}.}
...
Los ejercicios propuestos son los mismos a los propuestos en la sección ~\ref{sec:ejercicios-js} pero en este
caso se programa a través de bloques visuales. \\
...
para poder hacer uso de el método del \emph{HAL API}. El código resultante es el siguiente:
...
Como se aprecia en la Figura ~\ref{fig:fl-scratch-code} los bloques tienen una estructura similar al código
...
pero devuelve una lista con valores numéricos debido a que el área de cobertura es de 180º.
...
Como para los ejercicios anteriores, ha sido necesario generar bloques personalizados de \emph{Blockly}, en este caso se han
...
características que se irán añadiendo en el entorno \emph{A-FRAME} que permitirán mejorar el rendimiento
...
\begin{itemize}
	...
	no soportan el entorno A-Frame.
\end{itemize}
