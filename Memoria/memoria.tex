...
de el crecimiento de la realidad virtual en la web mediante el entorno \emph{AFRAME}.\\


El proyecto pretende crear una herramienta educativa, \emph{WebSim}, para la plataforma JdeRobot Kids
y el desarrollo de dos aplicaciones web para resolver ejercicios de \emph{visión artificial y 
programación de robots} mediante programación en \emph{JavaScript} y mediante el uso de bloques visuales
con \emph{Blockly}. \\

Para el desarrollo del simulador \emph{WebSim} se han utilizado diversas herramientas.
\begin{itemize}
...
\emph{AFRAME} framework. \\


The project aims to be an educational tool, \emph{WebSim}, for the JdeRobot Kids platform and develop
two web applications where students can solve \emph{artificial vision and robotic programming} 
problems using \emph{JavaScript} or using visual blocks from \emph{Blockly}. \\


Different tools have been used to develop the \emph{WebSim} simulator.
\begin{itemize}
...
la plataforma educativa \emph{JdeRobot-Kids} por el simulador 
...
simultánea con un servidor de bajo coste computacional.
...
sección ~\ref{sec:motivacion} el proyecto se orienta a la creación de una 
...
\begin{figure}[h]
  \centering
  \includegraphics[width=13cm, keepaspectratio]{img/Arquitectura-websim.png}
...
Como se comentó en la sección ~\ref{sec:motivacion} el objetivo del siguiente proyecto es crear 
la base de una aplicación para simular robots en el entorno \emph{AFRAME} permitiendo conectar un 
editor de código para ejecutar instrucciones con el robot simulado.
...
\begin{itemize}
	\item Como se muestra en la figura ~\ref{fig:arquitectura} una de las dependencias de \emph{WebSim} es el 
	entorno \emph{AFRAME}, para la simulación del robot ha sido necesario crear tres componentes llamados
	\emph{followBody, spectatorComponent, intersectionHandler} y es \emph{WebSim} el encargado de 
	...
	\emph{HAL API} \emph{(Hardware Abstraction Layer)} lo que simplifica mucho el uso del robot
	...
	más ya que el usuario solo tiene que enviar haciendo uso del objeto \emph{myRobot}
	y segundo que evitamos la creación de dos instancias del mismo objeto que llevaría a una incorrecta 
	ejecución.
	...
	está contenido en la variable global de \emph{WebSim} llamada \emph{myRobot}. Esto permite 
	conectar distintos tipos de 'puertas' para ejecutar código en el robot simulado como pueden ser
	las interfaces \emph{ROS Y ICE}.	
...
Como hemos comentado en el capítulo ~\ref{chap:herramientas} la etiqueta \emph{a-scene} es la 
...
Las etiquetas \emph{a-assets} son un modo de \emph{AFRAME} de gestionar de manera más eficiente 
...
\begin{figure}
	...
	\caption{Imagen del robot simulado en la escena, se resalta la cámara integrada en el robot en la imagen.}
...
\emph{positionSensor} junto con el componente \emph{followBody}. La carga de estos elementos
se hace desde WebSim, es necesario hacerlo así para poder modificarlos dinámicamente y añadir
más o menos sensores al robot. En la figura ~\ref{fig:pibot-simulado} se muestra el robot simulado y una muestra
...
que destacaremos dos \emph{spectator y wasd-controls-enabled}.
...
En esta sección se explicará el empaquetado final de \emph{WebSim}. Para empaquetarlo se ha hecho 
